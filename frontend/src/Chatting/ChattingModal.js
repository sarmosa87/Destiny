...
const ChattingModal = ({ users, onClose }) => {
  ...
  useEffect(() => {
    ...
    axios.get(`http://localhost:8081/api/messageList/${user.id}/${users.receiverId}`)
      .then(response => {
    ...
    // 메시지 수신 구독 설정
    const messageChannel = `/topic/messages/${user.id}`;
